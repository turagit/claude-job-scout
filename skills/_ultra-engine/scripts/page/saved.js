...
await sleep(2500);
const savedTab = [...document.querySelectorAll('[role="radio"],[role="tab"],button,a,label,div')].find(e => e.children.length <= 3 && /^Saved\s*·\s*\d+$/.test((e.innerText || e.textContent || '').trim()));
let savedCount = null;
if (savedTab) { savedCount = parseInt((savedTab.innerText || savedTab.textContent).replace(/\D/g, ''), 10); const host = savedTab.closest('[role="radio"],[role="tab"],button,a') || savedTab; if (host.getAttribute('aria-checked') !== 'true' && host.getAttribute('aria-selected') !== 'true') { if (host.click) { host.click(); await sleep(1500); } } }
