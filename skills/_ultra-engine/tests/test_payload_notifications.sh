...
assert_eq "1001 1002 1004" "$(echo "$out" | jq -r '[.results[].id]|join(" ")')" "today only, tier order, malformed posted_at handled"
assert_json_eq '{"a":1,"b":2,"c":0,"d":1,"total":4}' "$(echo "$out" | jq -c .tier_counts)" "tier counts"
...
assert_eq "false" "$(echo "$out" | jq -r '.results[2].fresh')" "B-tier malformed posted_at is not fresh"
assert_eq "null" "$(echo "$out" | jq -r '.results[2].confidence // "null"')" "absent optional field omitted"
