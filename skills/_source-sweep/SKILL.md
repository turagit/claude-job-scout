...
- **`html`** — read-only `WebFetch` `GET` of the source's `endpoint` (its listing/search page; the source's `url` is the human-facing homepage); parse the rendered listings into candidates (title + tags + the listing URL + any inline snippet). The **client-side full-text filter** (§ Client-side filter) runs here, on this list-level text, before extraction.
