...
- **Migration note:** `check-job-notifications` (html and markdown variants) still interpolates `{{ note.source }}` directly rather than through `source_chip()` — by design, the daily driver only ever tags a bare LinkedIn surface string (`Job Alert`/`Top Picks`/`Saved`/`Similar`), never the structured `{lane, provider, board}` object, so the two render identically today; converting it to the structured shape remains a deferred item (see `docs/ROADMAP.md`). The `deep-sweep` templates this note originally also named no longer exist — v0.15.0 retired `/deep-sweep` to a redirect alias with no template of its own.
