...
4. **Near-miss rail (Phase 14).** `near_misses[]` — entries whose single failed gate the rubric would otherwise have cleared A/B, each carrying `would_be_tier`, `failed_gate: {kind, detail}`, and `bend_hint` (`/bend <id>`). Rendered in a collapsed "would you bend?" rail, never among `results[]`.
5. **Scorecard strip (Phase 14).** `scorecard` — the embedded per-run accounting object; templates render its `.disclosures[]` as the "Run scorecard" strip.
