...
- A delta whose fingerprint **collides** with an already-merged entry resolves to one canonical "apply here" winner by the `ATS > LinkedIn > aggregator > marketplace` preference; the loser is recorded as a **sighting on the winner's `also_seen_on[]`** (`{name, url, lane}`), never re-fetched or re-scored. This is why the dispatcher — not the subagent — picks the canonical winner: only the dispatcher sees deltas from all sources at merge time (a single sweep sees only its own source).
