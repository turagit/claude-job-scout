...
argument-hint: [linkedin | external | source <name> | super — omit for the full-market sweep]
...
/ultramode super              — every enabled source, no rotation (all extension-lane sources; resumable)
```
...
- **Registry present** → Step 4, with the scope: bare = LinkedIn + all external; `linkedin` = the adapter only; `external` = registry sources only; `source <name>` = that one source only; `super` = LinkedIn + all external + **all** extension-lane sources (no rotation).
...
If `sources.json` has `schema_version < 2`, run `python3 $SCRIPTS/migrate_sources.py $WS/sources.json` first (idempotent — Phase 16). Load `sources.json`; derive poll order per `ultramode-sources.md` § Adaptive priority order. If `ultramode.registry_built_at` is older than 90 days, print one line — `Registry last rebuilt {{date}} (>90 days) — consider /sources rebuild.` — and continue (never block a read-only sweep). Pick this run's extension-lane subset: bare → `bash $SCRIPTS/rotation.sh pick $WS/sources.json 4` (unchanged); `super` → `bash $SCRIPTS/rotation.sh pick-all $WS/sources.json` with `rotated_out: []` (every enabled extension-lane source is picked — nothing is rotated out of a super sweep). Write `{picked, rotated_out, mode}` (rotated_out = extension sources not picked) to `$rd/rotation.json`, where `mode` ∈ `bare|linkedin|external|source|super` records this run's scope (the scorecard's accounting block reads it). BENELUX weighting lives inside `rotation.sh` — never re-order the pick in prose. The `category: "linkedin"` entry is never in this pick and never in `rotated_out` — it is swept unconditionally in 4d (and never `mark`ed). Scopes: `linkedin` skips the rotation pick and every non-LinkedIn source (write `{"picked": [], "rotated_out": [], "mode": "linkedin"}`); `external` proceeds as written but skips the adapter in 4d; `source <name>` replaces both the poll order and the rotation with exactly that one source (an extension-lane pick of one; still `rotation.sh mark` it when swept); under `super`, Step 4c and the derived poll order are unchanged from bare (every non-extension source is swept as usual); ONLY the extension-lane subset changes — `rotation.json .picked` comes from `bash $SCRIPTS/rotation.sh pick-all $WS/sources.json` (order is the script's, never re-derived in prose), with `rotated_out: []`. An unknown `source <name>` lists the registry names and stops. Build the snapshot: `bash $SCRIPTS/snapshot.sh $WS/tracker.json $WS/cache/ultramode-snapshot.json`, then `bash $SCRIPTS/checkpoint.sh save $rd snapshot $WS/cache/ultramode-snapshot.json`. ATS coverage in the sweep is the registry's `ats-provider` sources themselves; the dynamic watchlist union (tracker A/B-tier employers + `companies_to_target[]` + curated seed + manual additions, per `../_source-sweep/SKILL.md` § ATS watchlist) is applied when the registry is (re)built — Phase 15's `/sources` command (§ rebuild) owns refreshing it. This weekly sweep pass reads the registry as-is.
...
3. **Similar-jobs expansion (any run whose scope swept LinkedIn):** for each THIS-RUN LinkedIn-sourced role scored `tier: "A"`, collect up to 5 IDs from its listing page's "Similar jobs" rail (extension, dedupe-before-extract against the snapshot), produce ONE supplemental envelope (stage `sweep-linkedin-similar`, `source.board: "Similar"`), validate → merge → gate + score that batch the same way. One round only — expansion roles never seed further expansion (the old /deep-sweep Step 8 cap, unchanged).

### Step 4g: Scorecard + payload
`bash $SCRIPTS/scorecard.sh $rd $WS/tracker.json $TODAY` then `bash $SCRIPTS/payload.sh $WS/tracker.json $rd $TODAY <n-sources-swept> > $rd/payload.json`; checkpoint both. The payload is the render input verbatim — do not hand-assemble or re-sort it. `scorecard.json.accounting` (mode, attempted, completed, login_blocked, failed, rotated_out) is embedded in the payload's `scorecard` field verbatim by `payload.sh` — the terminal summary (Step 4h) and the report both read it from there, never recomputed.

### Step 4h: Render — ALWAYS
Follow `render-orchestration.md` with `view: "ultramode"` and `$rd/payload.json` (Hard Rule #8 — via `_visualizer`, never inline). This step runs even when 4c–4f partially failed: the report states what completed and the scorecard's disclosures name what didn't. `bash $SCRIPTS/checkpoint.sh save $rd render` only after the report file exists. Summary line: `✓ Ultramode — {{N_sources}} sources · {{N_new}} new roles — A:{{a}} B:{{b}} C:{{c}} · Filtered:{{gated}} · Near-misses:{{nm}} — report delivered`, followed by the accounting line: `Sources: {{attempted}} attempted · {{completed}} completed · {{login_blocked}} login-blocked · {{failed}} failed · {{rotated_out}} rotated out ({{mode}} mode)` (from `payload.scorecard.accounting`), and when `login_blocked > 0`, one line per blocked source with its rerun command (the `login_required` envelope's `message`, which already names the source and the `/ultramode source <name>` rerun), then every scorecard disclosure line. **Direct links (Phase 16, D15):** immediately after the disclosure lines, print one line per this-run A/B/C-tier merged tracker entry — `{{tier}} · {{title}} — {{company}} → {{url}}` (the entry's canonical `url` field) — matching Step E's format in `render-orchestration.md`, so every surfaced role is reachable without opening the HTML.
