...
```json
{
  "task": "render-report",
  "inputs": {
    "view": "match-jobs | job-search | check-job-notifications | funnel-report | check-inbox | interview-prep | ultramode",
...
`view` must be one of the allowed values listed in the input shape above (`match-jobs`, `job-search`, `check-job-notifications`, `funnel-report`, `check-inbox`, `interview-prep`, `ultramode`). `format` must be one of `html`, `markdown`, `plain`.
