...
**This skill is dispatched only by `_cv-optimizer/SKILL.md`, via the `Agent` tool, per `../shared-references/subagent-protocol.md`.** It is not user-invocable.
...
`role_id` is a deterministic slug generated by the orchestrator: `<company-slug>-<start-year>-<end-year>` (e.g., `acme-corp-2021-2024`). If the role is current (no end year yet), use `present` (e.g., `acme-corp-2024-present`). Company slugs are lowercased, with spaces replaced by hyphens and non-alphanumeric characters stripped. The orchestrator is responsible for generating stable IDs across invocations so fan-in merges correctly.
...
Apply the rules in `../_cv-optimizer/references/phase-3-optimized-rewrite.md` (SPAR bullet formula, persuasion-technique mapping, loss-aversion framing, anchoring). This subagent loads that reference on entry.
...
This skill has no slash-command surface and should not be wired to one. Dispatch is always via `Agent` from `_cv-optimizer/SKILL.md`.
