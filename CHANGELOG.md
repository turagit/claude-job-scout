...
- **Curated lane seed** (`ultramode-sources.md`) — a version-controlled, lane-tagged set of pre-verified sources that bootstraps the two categories a cold first-run cannot otherwise reach: **14 keyless ATS company boards** (Canonical, Grafana Labs, GitLab, N26, Wise, Datadog, Adyen, Mollie, …) and a **freelance-marketplace beachhead** (Malt, YunoJuno, Worksome, Braintrust, hackajob, IT-Contracts.nl, freelance.nl, …), plus a lane-specific keyless-feed seed (JustJoin.it, Landing.jobs, kube.careers, Arc.dev). Full provenance — every entry probed live 2026-06-15/16 — in `docs/superpowers/specs/2026-06-16-phase-13-verified-sources-research.json`. **Each seed is re-probed live before admission** (ATS via `resolve_ats` + an identity check that defeats slug collisions; marketplaces/feeds via the §2 gates), so the never-fabricate invariant is preserved: a seed is a candidate, the live probe is the admission. Lane-tagged for genericity — a non-tech lane (doctor, lawyer) simply does not resolve the tech seed and falls back to the professional-body and web-search axes.
- **Dispatch-enforcement + acceptance gates** (`ultramode/SKILL.md` Step 3e) — the dispatcher now **refuses to persist** a registry without a parsed `_source-discovery` delta from an actual `Agent` call (resolving the backbone from the reference is no longer a silent substitute for discovery); asserts a **count invariant** (`sources == backbone + fragment + user_sources`) so a dropped fragment fails loudly; and applies a **lane-conditional acceptance threshold** — a freelance lane must surface ≥1 freelance-marketplace **and** ≥1 ATS board (plus ≥5 non-backbone total), else it warns, names the dry lanes, and offers an immediate re-dispatch rather than writing a thin registry silently. `registry_built_at` is stamped **only** after a gated write.
- **Onboarding target-company seeding** (`ultramode/SKILL.md` Step 3c-bis) — onboarding now optionally collects 2–5 target companies (or derives candidates from the CV's employer history) into `requirements.companies_to_target[]`, so the ATS resolver has a non-empty seed on the very first run instead of waiting for LinkedIn sweeps.
- **Auditable-failure signals** (`canonical-schemas.md`, `_source-discovery`) — new `errors[]` codes `probe_failed` and `tool_unavailable` (a dead probe lane, distinct from a genuinely `lane_dry` one) and `discovered_below_threshold`, plus a mandatory rule that an empty/thin confirmed-set **must** populate `errors[]` naming every dry lane with round/probe counts. A `status: ok` with empty `sources[]` and empty `errors[]` is now a defined defect, not an accepted "dry" result.

### Changed

- **Gate A — a login wall is no longer a dead host** (`discovery-protocol.md`) — a recognised `401`/`403`/anti-bot challenge or login page on a plausibly-real job source is now **routed to the `extension` lane and retained**, not dropped on the bare non-2xx. This recovers the freelance-marketplace category (Malt/Toptal/Proxify-class) a naive "403 → drop" wrongly discarded. A true outright failure (timeout/`ECONNREFUSED`/blanket block) is recorded as `probe_failed`, never a silent drop.
- **Gate B — source admission is occupation-level, not contract-type-level** (`discovery-protocol.md`) — discovery now admits a board that is live and serves the **occupation** even when *today's* listings skew the "wrong" contract type, recording the skew in `notes`. Contract-type filtering moves to **sweep time** (`_source-sweep` client-side filter + the `contract_type` deal-breaker in `_gate-engine`, where `derive_priority_order` already plans to contract-filter). This stops structurally-valid boards being dropped because this week's roles happen to be permanent — the exact failure behind "9 roles found, all gated out".
- **Discovery is dispatched as a multi-round web loop** — `budget_lines` raised (200 → 800) to match a `WebSearch`-per-axis + `WebFetch`-per-candidate loop, with a mandatory loop-until-`ok` (never persist a `partial`; an empty-after-one-round `ok` or a `tool_unavailable` is re-probed before persisting). Jobicy added to the always-on backbone (keyless JSON + RSS, occupation-agnostic).

### Fixed

- First-run `/ultramode` discovery returning a **backbone-only registry** (the reported "why are we only reading from 8 sources?") — caused by an unenforced dispatch, two structurally-dark categories, and a silent-failure swallow that made "discovery never ran", "probe tools were dead", and "the lane is genuinely thin" all render as the identical 10-source file. All three now fail loudly, seed from verified provenance, or both.
