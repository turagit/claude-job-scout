...
- [ ] **`_ultra-engine` skill** — scripts: `snapshot`, `fingerprint`, `namespace-id`, `merge-tracker`, `validate-delta`, `checkpoint`, `rotation`, `jd-queue`, `scorecard`, `payload`; SKILL.md documents each contract.
- [ ] **Test suite** — `skills/_ultra-engine/tests/run.sh` (the repo's first automated tests); CLAUDE.md testing section updated.
- [ ] **`_source-sweep` verbatim prompt templates** (api/rss/html) — dispatcher fills declared variables only; caps parameterised + always disclosed.
- [ ] **Sweep-time explicit-violation drop** (D1/D4) — gate semantics passed into sweeps as data; detachering-class values honoured from `deal_breakers[].values`.
- [ ] **Fetch-then-gate + JD budget** (D2/D9) — budgeted full-JD fetch for unconfirmed-data failures; deferred queue persisted; scoring requires `jd_path`.
- [ ] **Near-miss rail** (D3) — structured `gate_violations[]` restored; `near_miss` rule (exactly one failed gate + A/B-grade dimensions); `_visualizer` collapsed rail.
- [ ] **`/bend <id>` command** (D13) — one-shot re-gate/re-score with the failed gate relaxed; `bent: true` audit trail; never edits `deal_breakers`.
- [ ] **Extension-lane rotation, actually swept** (D8) — staleness-ordered N=4/run; `last_swept_at` bookkeeping; checkpoint per source.
- [ ] **Checkpoints + always-render + resume** (D7) — kill-test: interrupt mid-scoring → resume → report still ships.
- [ ] **Per-run scorecard** (D12) — swept/skipped/rotated, seen/new/gated-by-reason/near-miss, fetch used vs deferred, tier yield, stage timings; embedded in report.
- [ ] **Migration/back-compat** — IDs immutable; `source` normalised on read; additive fields only.
- [ ] **Release v0.14.0** — version bump, CHANGELOG, README notes, this section ticked.

## Phase 15 — v0.15.0: The ultramode merge

One verb for the whole market. Follows Phase 14; same spec.

- [ ] **`/ultramode` flagship + scopes** — bare = LinkedIn + external unified; `linkedin` / `external` / `source <name>` scoped forms; LinkedIn becomes a registry source entry.
- [ ] **`/sources`** — `list` / `add <url|name>` / `rebuild` / first-run lane interview (absorbs `/ultramode sources*` + `onboarding`).
- [ ] **`/tune`** — vocabulary + gates display and edits; cache-invalidation contract for profile-input changes.
- [ ] **Retirement** — `/deep-sweep` deprecation alias; `ultramode.default` removed with config migration note.
- [ ] **Docs rewrite** — README/QUICKSTART around `/analyze-cv` → `/sources` → `/ultramode` weekly → `/check-job-notifications` daily → `/tune` between runs.
- [ ] **Fresh-workspace end-to-end + publish gate** — scratch workspace interview→registry→sweep→report; two consecutive clean weekly runs on CVFREELANCER + CVDIRECTOR; publish-readiness pass.
- [ ] **Release v0.15.0** — version bump, CHANGELOG, this section ticked.
