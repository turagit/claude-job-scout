...
- [ ] **Step 4: Scope-gate Step 4 and slot the adapter in.** Three surgical edits inside the existing `## Step 4: Sweep flow (the multi-source pass)`:
  ...
  4. In **Step 4f** sub-step 2, append to its final sentence: `For LinkedIn-sourced roles, then complete the query-stats second half (tier counts, retirement, synonym promotion) per \`references/linkedin-adapter.md\` § The sweep, item 6.`
